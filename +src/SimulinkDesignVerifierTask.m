classdef SimulinkDesignVerifierTask < src.ModelBasedDesignTask
    properties
        Model
    end
    ...
    methods (Static)
        function group = forEachModel(models, options)
            ...
            tasks = src.SimulinkDesignVerifierTask.empty();
            for i = 1:numel(models)
                m = models(i);
                tasks(end+1) = src.SimulinkDesignVerifierTask(m, args{:}, Title=m); %#ok<AGROW>
            end

            group = matlab.buildtool.TaskGroup(tasks, ...
                TaskNames=models, ...
                Title="Simulink Design Verifier Tasks", ...
                Dependencies=deps);
        end
    end

    methods
        function task = SimulinkDesignVerifierTask(model, options)
            arguments
                model 
            ...
            task.Outputs = fullfile("results", "SimulinkDesignVerifierTask", strcat(model,".txt"));
