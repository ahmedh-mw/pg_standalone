classdef SimulinkCodeInspectorTask < src.ModelBasedDesignTask
    properties
        Model
    end
    ...
    methods (Static)
        function group = forEachModel(models, options)
            ...
            tasks = src.SimulinkCodeInspectorTask.empty();
            for i = 1:numel(models)
                m = models(i);
                tasks(end+1) = src.SimulinkCodeInspectorTask(m, args{:}, Title=m); %#ok<AGROW>
            end

            group = matlab.buildtool.TaskGroup(tasks, ...
                TaskNames=models, ...
                Title="Simulink Code Inspector Tasks", ...
                Dependencies=deps);
        end
    end

    methods
        function task = SimulinkCodeInspectorTask(model, options)
            arguments
                model 
            ...
            task.Outputs = fullfile("results", "SimulinkCodeInspectorTask", strcat(model,".txt"));
